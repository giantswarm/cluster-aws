...
| `global.apps.clusterAutoscalerCrossplaneResources.extraConfigs` | **Extra config maps or secrets** - Extra config maps or secrets that will be used to customize to the app. The desired values must be under configmap or secret key 'values'. The values are merged in the order given, with the later values overwriting earlier, and then inline values overwriting those. Resources must be in the same namespace as the cluster.|**Type:** `array`<br/>|
| `global.apps.clusterAutoscalerCrossplaneResources.extraConfigs[*]` | **Config map or secret**|**Type:** `object`<br/>|
| `global.apps.clusterAutoscalerCrossplaneResources.extraConfigs[*].kind` | **Kind** - Specifies whether the resource is a config map or a secret.|**Type:** `string`<br/>**Allowed values:** `ConfigMap`, `Secret`<br/>|
| `global.apps.clusterAutoscalerCrossplaneResources.extraConfigs[*].name` | **Name** - Name of the config map or secret. The object must exist in the same namespace as the cluster App.|**Type:** `string`<br/>|
| `global.apps.clusterAutoscalerCrossplaneResources.extraConfigs[*].optional` | **Optional** - Optional marks this ValuesReference as optional. When set, a not found error for the values reference is ignored, but any ValuesKey, TargetPath or transient error will still result in a reconciliation failure.|**Type:** `boolean`<br/>|
| `global.apps.clusterAutoscalerCrossplaneResources.values` | **Values** - Values to be passed to the app. Values will have higher priority than values from configmaps.|**Type:** `object`<br/>|
| `global.apps.clusterProbes` | **App** - Configuration of a default app that is part of the cluster and is deployed as a HelmRelease resource.|**Type:** `object`<br/>|
| `global.apps.clusterProbes.extraConfigs` | **Extra config maps or secrets** - Extra config maps or secrets that will be used to customize to the app. The desired values must be under configmap or secret key 'values'. The values are merged in the order given, with the later values overwriting earlier, and then inline values overwriting those. Resources must be in the same namespace as the cluster.|**Type:** `array`<br/>|
| `global.apps.clusterProbes.extraConfigs[*]` | **Config map or secret**|**Type:** `object`<br/>|
| `global.apps.clusterProbes.extraConfigs[*].kind` | **Kind** - Specifies whether the resource is a config map or a secret.|**Type:** `string`<br/>**Allowed values:** `ConfigMap`, `Secret`<br/>|
| `global.apps.clusterProbes.extraConfigs[*].name` | **Name** - Name of the config map or secret. The object must exist in the same namespace as the cluster App.|**Type:** `string`<br/>|
| `global.apps.clusterProbes.extraConfigs[*].optional` | **Optional** - Optional marks this ValuesReference as optional. When set, a not found error for the values reference is ignored, but any ValuesKey, TargetPath or transient error will still result in a reconciliation failure.|**Type:** `boolean`<br/>|
| `global.apps.clusterProbes.values` | **Values** - Values to be passed to the app. Values will have higher priority than values from configmaps.|**Type:** `object`<br/>|
...
| `global.apps.externalDnsCrossplaneResources.extraConfigs` | **Extra config maps or secrets** - Extra config maps or secrets that will be used to customize to the app. The desired values must be under configmap or secret key 'values'. The values are merged in the order given, with the later values overwriting earlier, and then inline values overwriting those. Resources must be in the same namespace as the cluster.|**Type:** `array`<br/>|
| `global.apps.externalDnsCrossplaneResources.extraConfigs[*]` | **Config map or secret**|**Type:** `object`<br/>|
| `global.apps.externalDnsCrossplaneResources.extraConfigs[*].kind` | **Kind** - Specifies whether the resource is a config map or a secret.|**Type:** `string`<br/>**Allowed values:** `ConfigMap`, `Secret`<br/>|
| `global.apps.externalDnsCrossplaneResources.extraConfigs[*].name` | **Name** - Name of the config map or secret. The object must exist in the same namespace as the cluster App.|**Type:** `string`<br/>|
| `global.apps.externalDnsCrossplaneResources.extraConfigs[*].optional` | **Optional** - Optional marks this ValuesReference as optional. When set, a not found error for the values reference is ignored, but any ValuesKey, TargetPath or transient error will still result in a reconciliation failure.|**Type:** `boolean`<br/>|
| `global.apps.externalDnsCrossplaneResources.values` | **Values** - Values to be passed to the app. Values will have higher priority than values from configmaps.|**Type:** `object`<br/>|
...
| `global.connectivity.certManager.createIamRole` | **Create cert-manager IAM role** - Set to false to opt out of provisioning the cert-manager IAM role via crossplane and bring your own role (with the conventional name `<clusterName>-cert-manager`).|**Type:** `boolean`<br/>**Default:** `true`|
| `global.connectivity.certManager.useDnsChallenges` | **Use DNS Challenges** - Set to true to enable DNS challenges in the default ClusterIssuer and install all necessary IAM roles.|**Type:** `boolean`<br/>**Default:** `true`|
...
| `global.controlPlane.rootVolumeSizeGB` | **Root volume size (GB)** - A certain minimum may be enforced depending on OS requirements.|**Type:** `integer`<br/>**Default:** `15`|
...
| `global.nodePools.PATTERN[option#1].architecture` | **CPU architecture** - Target CPU architecture for the node pool. Selects the matching CAPI Flatcar AMI: x86_64 nodes use the unsuffixed AMI name, arm64 nodes use the AMI with `arm64-` infix. The instance type must match (e.g. m7g.xlarge / t4g.medium for arm64, m5.xlarge / t3.medium for x86_64). For arm64 pools, also add a `kubernetes.io/arch=arm64:NoSchedule` taint via `customNodeTaints` so amd64-only workloads don't land here.|**Type:** `string`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Allowed values:** `x86_64`, `arm64`<br/>**Default:** `"x86_64"`|
...
| `global.nodePools.PATTERN[option#1].consolidationBudgets` | **Disruption budgets (deprecated)** - Deprecated: Use disruptionBudgets instead.|**Type:** `array`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>|
...
| `global.nodePools.PATTERN[option#1].disruptionBudgets` | **Disruption budgets** - Budgets control the speed Karpenter can scale down nodes. Karpenter will respect the minimum of the currently active budgets, and will round up when considering percentages. Duration and Schedule must be set together.|**Type:** `array`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Default:** `[{"nodes":"10%"}]`|
| `global.nodePools.PATTERN[option#1].disruptionBudgets[*]` | **Budget**|**Type:** `object`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>|
| `global.nodePools.PATTERN[option#1].disruptionBudgets[*].duration` | **Duration** - Determines how long a Budget is active since each Schedule hit. Only minutes and hours are accepted, as cron does not work in seconds. If omitted, the budget is always active.|**Type:** `string`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Value pattern:** `^((([0-9]+(h|m))|([0-9]+h[0-9]+m))(0s)?)$`<br/>|
| `global.nodePools.PATTERN[option#1].disruptionBudgets[*].nodes` | **Nodes** - Dictates the maximum number of NodeClaims owned by this NodePool that can be terminating at once. This is calculated by counting nodes that have a deletion timestamp set, or are actively being deleted by Karpenter. This field is required when specifying a budget.|**Type:** `string`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Value pattern:** `^((100|[0-9]{1,2})%|[0-9]+)$`<br/>**Default:** `"10%"`|
| `global.nodePools.PATTERN[option#1].disruptionBudgets[*].reasons` | **Disruption reasons** - List of disruption methods that this budget applies to. If Reasons is not set, this budget applies to all methods.|**Type:** `array`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>|
| `global.nodePools.PATTERN[option#1].disruptionBudgets[*].reasons[*]` |**None**|**Type:** `string`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Allowed values:** `Underutilized`, `Empty`, `Drifted`<br/>|
| `global.nodePools.PATTERN[option#1].disruptionBudgets[*].schedule` | **Schedule** - Specifies when a budget begins being active, following the upstream cronjob syntax. If omitted, the budget is always active. Timezones are not supported.|**Type:** `string`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Value pattern:** `^(@(annually|yearly|monthly|weekly|daily|midnight|hourly))|((.+)\s(.+)\s(.+)\s(.+)\s(.+))$`<br/>|
...
| `global.nodePools.PATTERN[option#1].limits.cpu` |CPU limits are described with a DecimalSI value|**Types:** `number`, `string`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Default:** `"1000"`|
...
| `global.nodePools.PATTERN[option#1].localNvme` | **Local instance-store NVMe ephemeral storage** - Use local EC2 instance-store NVMe disks (RAID0 if multiple) for the kubelet directory so pod `emptyDir` and the container runtime use fast, node-local storage instead of the EBS root/lib volume. Only takes effect on instance types that actually have instance store (e.g. the `i3en`/`i4i`/`*d` families); it is a safe no-op otherwise. Cluster-wide parameters (mount path, eviction threshold) are configured under the `cluster` chart's `internal.advancedConfiguration.workers.localEphemeralStorage`.|**Type:** `object`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>|
| `global.nodePools.PATTERN[option#1].localNvme.enabled` | **Enable**|**Type:** `boolean`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Default:** `false`|
...
| `global.nodePools.PATTERN[option#1].rootVolumeSizeGB` | **Root volume size (GB)** - A certain minimum may be enforced depending on OS requirements.|**Type:** `integer`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Default:** `15`|
...
| `global.nodePools.PATTERN[option#2].architecture` | **CPU architecture** - Target CPU architecture for the node pool. Selects the matching CAPI Flatcar AMI: x86_64 nodes use the unsuffixed AMI name, arm64 nodes use the AMI with `arm64-` infix. The instance type must match (e.g. m7g.xlarge / t4g.medium for arm64, m5.xlarge / t3.medium for x86_64). For arm64 pools, also add a `kubernetes.io/arch=arm64:NoSchedule` taint via `customNodeTaints` so amd64-only workloads don't land here.|**Type:** `string`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Allowed values:** `x86_64`, `arm64`<br/>**Default:** `"x86_64"`|
...
| `global.nodePools.PATTERN[option#2].localNvme` | **Local instance-store NVMe ephemeral storage** - Use local EC2 instance-store NVMe disks (RAID0 if multiple) for the kubelet directory so pod `emptyDir` and the container runtime use fast, node-local storage instead of the EBS root/lib volume. Only takes effect on instance types that actually have instance store (e.g. the `i3en`/`i4i`/`*d` families); it is a safe no-op otherwise. Cluster-wide parameters (mount path, eviction threshold) are configured under the `cluster` chart's `internal.advancedConfiguration.workers.localEphemeralStorage`.|**Type:** `object`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>|
| `global.nodePools.PATTERN[option#2].localNvme.enabled` | **Enable**|**Type:** `boolean`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Default:** `false`|
...
| `global.nodePools.PATTERN[option#2].rootVolumeSizeGB` | **Root volume size (GB)** - A certain minimum may be enforced depending on OS requirements.|**Type:** `integer`<br/>**Key pattern:**<br/>`PATTERN`=`^[a-z0-9][-a-z0-9]{3,18}[a-z0-9]$`<br/>**Default:** `15`|
