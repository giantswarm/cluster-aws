...
| `global.connectivity.network.vpcCidr` | **VPC CIDR** - IPv4 address range to assign to this cluster's VPC, in CIDR notation. **DEPRECATED**, please use `global.connectivity.network.vpcCidrs`.|**Type:** `string`<br/>**Default:** `""`|
| `global.connectivity.network.vpcCidrs` | **VPC CIDRs**|**Type:** `array`<br/>**Default:** `["10.0.0.0/16"]`|
| `global.connectivity.network.vpcCidrs[*]` | **VPC CIDR** - IPv4 address range to assign to this cluster's VPC, in CIDR notation. The first-listed CIDR should not be changed because this could lead to unexpected reconciliation behavior. Only /16 to /28 sizes are supported (see https://docs.aws.amazon.com/vpc/latest/userguide/vpc-cidr-blocks.html).|**Type:** `string`<br/>**Example:** `["10.0.0.0/16"]`<br/>**Value pattern:** `^(([0-9]{1,3}\.){3}[0-9]{1,3}/(1[6-9]|2[0-8]))$`<br/>|
