...
| `global.connectivity.dns.delegationRoleARN` | **Delegation AWS IAM Role** - AWS IAM Role to use when taking care of creating the DNS delegation between the cluster hosted zone and its parent hosted zone. It needs to have permissions to manage the parent hosted zone.|**Type:** `string`<br/>**Example:** `"arn:aws:iam::123456789012:role/DNSDelegationRole"`<br/>**Value pattern:** `^arn:(aws|aws-cn|aws-us-gov):iam::\d{12}:role\/.+$`<br/>|
