...
instanceType: {{ .Values.global.connectivity.bastion.instanceType }}
cloudInit: {}
...
subnet:
  filters:
    - name: tag:{{ if eq .Values.global.connectivity.vpcMode "private" }}github.com/giantswarm/aws-vpc-operator/role{{else}}sigs.k8s.io/cluster-api-provider-aws/role{{end}}
      values:
      - {{ if eq .Values.global.connectivity.vpcMode "private" }}private{{else}}public{{end}}
    - name: tag:{{ if eq .Values.global.connectivity.vpcMode "private" }}github.com/giantswarm/aws-vpc-operator/{{else}}sigs.k8s.io/cluster-api-provider-aws/cluster/{{end}}{{ include "resource.default.name" $ }}
      values:
      - owned
      - shared
    {{- range $i, $tags :=  .Values.global.connectivity.bastion.subnetTags }}
